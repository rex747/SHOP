...
private:
    ...
    void onLoginResult() {
        ...
        case RESULT_NOT_FOUND: {
            ...
            if (passwordOk) {
                // =================================================================
                // РЕГИСТРАЦИЯ КАССИРА (ИЗМЕНЕНИЕ):
                // Окно выбора роли «ДА — Директор / НЕТ — Товаровед» УДАЛЕНО.
                // В клиентской части КАССИРА после ввода пароля администратора
                // СРАЗУ открывается окно регистрации КАССИРА (роль "cashier").
                // Серверная часть регистрации СОХРАНЕНА: тот же эндпоинт
                // /api/v1/clients/register, тот же механизм генерации пароля.
                // =================================================================
                g_logger.info(L"LoginWindow: admin password correct, creating "
                    L"WorkerRegistrationWindow (role=cashier) for " + m_normalizedPhone);
                const std::string registrationRole = "cashier";
                WorkerRegistrationWindow regWnd(m_normalizedPhone, registrationRole);
                int result = regWnd.show(m_hWnd);
                if (result == IDOK) {
                    g_logger.info(L"LoginWindow: cashier registration successful, "
                        L"performing auto-login for " + m_normalizedPhone);
                    // Берём СГЕНЕРИРОВАННЫЙ сервером пароль из окна регистрации
                    ...
                    auto response = g_httpsClient.post(
                        L"/api/v1/clients/by_phone", request, L"");
                    if (response && response->contains("id") &&
                        ...
                        std::wstring fullName =
                            utf8_to_wstring((*response)["name"].get<std::string>());
                        ...
                        int64_t expiresAt =
                            ...
                            (*response)["expires_at"].is_number_integer()
                            ? (*response)["expires_at"].get<int64_t>()
                            : std::chrono::duration_cast<std::chrono::seconds>(
                                std::chrono::system_clock::now()
                                .time_since_epoch()).count() + 3600;
                        // Сохраняем роль из ответа сервера ("cashier")
                        if (response->contains("role") &&
                            (*response)["role"].is_string()) {
                            g_authManager.setRole(
                                (*response)["role"].get<std::string>());
                        }
                        g_authManager.setAuthTokens(accessToken, refreshToken,
                            expiresAt, wstring_to_utf8(m_normalizedPhone));
                        g_authManager.setLoggedIn(true, clientId, fullName,
                            m_normalizedPhone);
                        m_clientId = clientId;
                        m_fullName = fullName;
                        m_loginSuccess = true;
                        g_logger.info(L"LoginWindow: cashier auto-login successful, "
                            L"clientId=" + std::to_wstring(clientId));
                        MessageBoxW(m_hWnd,
                            L"Регистрация кассира завершена, вход выполнен!",
                            L"Успех", MB_OK);
                        DestroyWindow(m_hWnd);
                    }
                    else {
                        g_logger.error(L"LoginWindow: cashier auto-login FAILED "
                            L"after registration for " + m_normalizedPhone);
                        MessageBoxW(m_hWnd,
                            L"Регистрация прошла, но не удалось выполнить вход. "
                            L"Войдите с выданным паролем.",
                            L"Ошибка", MB_OK);
                        m_isAuthenticating = false;
                        EnableWindow(m_hSubmitBtn, TRUE);
                        for (HWND hBtn : m_allKeyButtons) EnableWindow(hBtn, TRUE);
                    ...
                    g_logger.info(L"LoginWindow: cashier registration CANCELLED "
                        L"by admin for " + m_normalizedPhone);
                ...
                g_logger.warning(L"LoginWindow: WRONG admin password entered "
                    L"for " + m_normalizedPhone);
    ...
    std::wstring normalizePhone(const std::wstring& input) {
        ...
        // 11 цифр, начинающихся с 7 или 8 (79006001010 / 89006001010)
        if (digits.length() == 11 && (digits[0] == L'7' || digits[0] == L'8')) {
            return L"+7" + digits.substr(1);
        }
        // 10 цифр (9006001010) — дополняем до +7
        if (digits.length() == 10) {
            return L"+7" + digits;
        }
        // Любые прочие варианты (в т.ч. 59006001010 — 11 цифр с первой '5')
        // отклоняются НА КЛИЕНТЕ до отправки на сервер
        return L"";
    }
