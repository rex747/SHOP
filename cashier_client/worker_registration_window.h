...
private:
    ...
    void onSubmit() {
        g_logger.info(L"[onSubmit] Starting registration for cashier client...");
        ...
        //Страховка на не верный формат телефона
        std::wstring normalizedPhone = RegUtils::normalizePhone(m_phone);
        ...
        if (response && response->contains("success") && (*response)["success"].get<bool>()) {
            ...
            if (response->contains("password") && (*response)["password"].is_string()) {
                ...
                // НОВОЕ: тексты для роли "cashier"
                std::wstring firstLine;
                const wchar_t* whoToGive;
                if (m_role == "director") {
                    firstLine = L"Директор магазина успешно зарегистрирован!";
                    whoToGive = L"директору";
                }
                else if (m_role == "cashier") {
                    firstLine = L"Кассир успешно зарегистрирован!";
                    whoToGive = L"кассиру";
                }
                else {
                    firstLine = L"Товаровед успешно зарегистрирован!";
                    whoToGive = L"товароведу";
                }
                ...
                g_logger.info(L"WorkerRegistrationWindow: generated password displayed "
                    L"to admin for " + m_phone + L", role=" + utf8_to_wstring(m_role));
                MessageBoxW(m_hWnd, passwordMsg.c_str(),
                    (m_role == "director") ? L"Пароль для входа директора"
                    : (m_role == "cashier") ? L"Пароль для входа кассира"
                    : L"Пароль для входа товароведа",
    ...
    static LRESULT CALLBACK WndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
        ...
        case WM_CTLCOLORSTATIC: {
            ...
            if (id == IDC_WORKER_REG_TITLE) {
                ...
                // ИСПРАВЛЕНО: было GetStockObject(NULL_BRUSH) — фон не стирался
                return (LRESULT)pThis->m_hWhiteBrush;
            }
            if (id == IDC_WORKER_REG_STATUS) {
                SetTextColor(hdc, RGB(211, 47, 47));
                // ИСПРАВЛЕНО: реальная кисть стирает предыдущий текст статуса,
                // исключая наслаивание надписей при каждом SetWindowTextW
                return (LRESULT)pThis->m_hWhiteBrush;
            }
            SetTextColor(hdc, RGB(0, 0, 0));
            // ИСПРАВЛЕНО: единообразно для всех STATIC (защита от будущего динамического текста)
            return (LRESULT)pThis->m_hWhiteBrush;
        }
