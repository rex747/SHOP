...
enum class QueueType {
    GENERAL,           // ќбща€ очередь (до 20 товаров)
    FIRST_TIME,        // ѕервый раз (договор комиссии)
    EXTRA_20,          // +20 позиций
    TRUST,            // Ќа доверии
    PAID,             // ѕлатный прием (200 руб)
    EXPENSIVE         // ƒорогой товар (>5000 руб)
};
...
public:
    ...
    std::optional<QueueTicket> getTrustAcceptance(int clientId, const std::wstring& authToken) {
        ...
        if (response && response->contains("success") && (*response)["success"].get<bool>()) {
            ...
            ticket.itemsCount = 1; // дл€ довери€ 1
