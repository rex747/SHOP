...
private:
    ...
    static constexpr int REFRESH_INTERVAL_MS = 5000; // 5 секунд
    ...
    void speak(const std::wstring& text) {
        if (m_pVoice) {
            std::wstring full = L"Клиент с номером " + text + L" пройдите к окну номер 1";
    ...
    void onAccept() {
        int sel = SendMessageW(m_hListBox, LB_GETCURSEL, 0, 0);
        if (sel == LB_ERR) {
            MessageBoxW(m_hWnd, L"Выберите талон из списка", L"Информация", MB_OK);
            return;
        }
        std::wstring ticketNumber;
        int len = SendMessageW(m_hListBox, LB_GETTEXTLEN, sel, 0);
        if (len > 0) {
        ...
        if (response && response->contains("success") && (*response)["success"].get<bool>()) {
            ...
            speak(ticketNumber);
            // Убираем из списка немедленно (или по таймеру через 5 мин)
            // Пока просто удаляем из списка, но через 5 минут он должен исчезнуть.
            // Лучше перезапросить список через refresh, но для UX сразу удалим локально.
            {
                std::lock_guard<std::mutex> lock(m_mutex);
            ...
            // Также запускаем таймер, чтобы через 5 минут убедиться, что он удалён с сервера
            // и обновить список.
            std::thread([this]() {
