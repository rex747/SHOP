...
private:
    ...
    void createControls() {
        ...
        HWND hSubmitBtn = CreateWindowExW(0, L"BUTTON", L"Зарегистрировать", WS_VISIBLE | WS_CHILD | BS_PUSHBUTTON | BS_NOTIFY, startX, btnY, btnWidth, btnHeight, m_hWnd, (HMENU)(INT_PTR)IDC_REG_SUBMIT, g_hInstance, nullptr);
        m_buttons.push_back(hSubmitBtn);

        HWND hBackBtn = CreateWindowExW(0, L"BUTTON", L"Назад", WS_VISIBLE | WS_CHILD | BS_PUSHBUTTON | BS_NOTIFY, startX + labelWidth + editWidth + 20 - btnWidth, btnY, btnWidth, btnHeight, m_hWnd, (HMENU)(INT_PTR)IDC_REG_BACK, g_hInstance, nullptr);
    ...
    void onSubmit() {
        ...
        if (response && response->contains("success") && (*response)["success"].is_boolean() && (*response)["success"].get<bool>()) {
            ...
            // ИСПРАВЛЕНО: Заменяем markAsRegistered на setupTOTP для инициализации TOTP-секрета на сервере
            auto totpResult = g_authManager.setupTOTP(normalizedPhone);
            if (totpResult.success) {
                g_logger.info(L"TOTP initialized successfully for: " + normalizedPhone);
            }
            else {
                g_logger.warning(L"TOTP initialization failed for: " + normalizedPhone + L". Client may need to set it up manually.");
            }
            ...
            if (response && response->contains("error") && (*response)["error"].is_string()) {
                ...
                errMsg = L"Нет связи с сервером. Проверьте подключение и настройки сети.";
            }
    ...
    LRESULT onCommand(int cmd) {
        switch (cmd) {
        case IDC_REG_SUBMIT: onSubmit(); break;
        case IDC_REG_BACK: DestroyWindow(m_hWnd); break;
        case IDC_REG_CLOSE: DestroyWindow(m_hWnd); break;
        }
        return 0;
    }
