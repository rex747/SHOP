...
private:
    ...
    std::string generateJWT(const std::string& phone, int expirySeconds) {
        ...
        // »справление: €вное создание std::string гарантирует наличие методов .data() и .size()
...
public:
    ...
    std::pair<std::string, std::string> generateTokens(const std::string& phone) {
        ...
        try {
            ...
            if (clientOpt) {
                int64_t now = std::chrono::system_clock::now().time_since_epoch().count() / 1000;
                // »справление: использование публичного метода вместо пр€мого доступа к приватному conn_
                db_->saveAuthTokens(
                    clientOpt->id,
