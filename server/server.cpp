...
public:
    ...
    void run() {
        // Асинхронное SSL-рукопожатие перед чтением HTTP
...
private:
    ...
    void handleRequest() {
        ...
        try {
            if (target == "/api/v1/clients/register" && method == http::verb::post) {
                ...
                handleAuthSMS();
                g_serverLogger.info("Request: " + std::string(request_.method_string()) + " " + std::string(target));
            }
    ...
    void handleClientRegister()
    {
        ...
        try {
            ...
            response_.set(http::field::content_type, "application/json");
            response_.body() = json{ {"success", false},
                                    {"error", "Invalid JSON"} }.dump();
            response_.prepare_payload();
        ...
        {  // <-- ФИГУРНАЯ СКОБКА ЗДЕСЬ!
            g_serverLogger.warning("Missing required fields in registration request: " + body.dump());
            response_.result(http::status::bad_request);
            response_.set(http::field::content_type, "application/json");
            response_.body() = json{ {"success", false},
                                    {"error", "Missing required fields"} }.dump();
            response_.prepare_payload();
            g_serverLogger.error("Missing required fields in registration request: " + body.dump());
            return;
        }
        ...
        // Валидация телефона на стороне сервера (защита от мусора)
        std::string digits;
    ...
    void handleOneCSync() {
		// Sync data with 1C  NB: исправить на асинхронный вызов для продакшена
    ...
    void onComplete() {
        ...
        // Асинхронное graceful shutdown SSL перед закрытием TCP
...
public:
    // Конструктор acceptor(ioc, endpoint) автоматически выполняет open, bind, listen.
    // Если порт занят или другая ошибка, будет выброшено исключение.
