// worker_window.h – ФИНАЛЬНАЯ ПРОДАКШН-ВЕРСИЯ (ИСПРАВЛЕНО ОТОБРАЖЕНИЕ ФИО И ТЕЛЕФОНА)
// =============================================================================
// ИСПРАВЛЕНИЯ ОТ 20.08.2026:
// 1. ИСПРАВЛЕНО ОТОБРАЖЕНИЕ ФИО И ТЕЛЕФОНА ПОСЛЕ СОХРАНЕНИЯ ДАННЫХ КОМИТЕНТА:
...
private:
    ...
    void loadClientDataFromServer(int clientId) {
        ...
        std::thread([this, clientId, authToken, hWndCopy]() {
            ...
            if (response && !response->contains("error")) {
                ...
                // Сохраняем данные под мьютексом
                {
                ...
                // Отправляем сообщение об ошибке (clientId = 0)
    ...
    void createItemInputControls(int left, int top, int width) {
        ...
        if (m_currentQueueType == L"first_time") {
            ...
            HWND hCommitBlockTitle = CreateWindowExW(0, L"STATIC",
                L"=== Данные нового комитента ===",
                WS_VISIBLE | WS_CHILD | SS_LEFT,
                left, top - 30, width, 30, m_hWnd, nullptr, g_hInstance, nullptr);
            SendMessageW(hCommitBlockTitle, WM_SETFONT, (WPARAM)m_hFont, TRUE);
            top += 10;
            ...
            // Создается ВСЕГДА для first_time, но видимость зависит от m_isCommitDataSaved
            // =================================================================
    ...
    void showItemInputForm(int clientId, const std::wstring& ticketNumber, const std::wstring& windowNumber, const std::wstring& clientName) {
        ...
        // =============================================================================
        // НОВОЕ: Загружаем данные клиента с сервера для всех очередей кроме first_time
    ...
    void onAccept() {
        ...
        if (response && response->contains("success") && (*response)["success"].get<bool>()) {
            ...
            if (m_currentQueueType == L"first_time") {
                ...
                showItemInputForm(clientId, ticketNumber, windowW, L"Клиент #" + std::to_wstring(clientId));
            }
    ...
    void onServe() {
        ...
        if (response && response->contains("success") && (*response)["success"].get<bool>()) {
            { 
                std::lock_guard<std::mutex> lock(m_mutex); 
                if (itemData < (LRESULT)m_tickets.size()) m_tickets.erase(m_tickets.begin() + itemData); 
                if (m_selectedIndex >= (int)m_tickets.size()) m_selectedIndex = (int)m_tickets.size() - 1; 
            }
            ...
            if ((m_currentQueueType == L"general" || 
                m_currentQueueType == L"extra_20" || 
                m_currentQueueType == L"trust" || 
                m_currentQueueType == L"paid" || 
                m_currentQueueType == L"expensive") && !m_tickets.empty()) {
        ...
        switch (msg) {
            ...
            case WM_COMMIT_SAVED: {
                ...
                if (clientId > 0) {
                    ...
                    // =====================================================================
                    // сохраняем ФИО и телефон ДО уничтожения полей ввода
                    wchar_t fioBuf[256];
                    ...
                    // =====================================================================
                    // Теперь пересоздаём контролы (поля ввода будут уничтожены)
                    pThis->repositionItemInputControls();
                    pThis->showItemInputControls(true);

                    // =====================================================================
                    // Устанавливаем сохранённые данные в поля отображения
                    ...
                    // Показываем поля отображения (они создаются скрытыми в createItemInputControls)
            ...
            // =============================================================================
            // ОБРАБОТКА ЗАГРУЗКИ ДАННЫХ КЛИЕНТА С СЕРВЕРА
            ...
            case WM_COMMAND: {
                ...
                if (id == ID_ITEM_BACK_BTN && code == BN_CLICKED) { 
                    if (MessageBoxW(pThis->m_hWnd, L"Вернуться? Несохранённые данные будут потеряны.", L"Подтверждение", MB_YESNO) == IDYES) pThis->returnToQueueList(); 
                    return 0; 
                }
    ...
    void createControls() {
        ...
        QueueTypeEntry entries[] = { 
            ...
            { 
                L"В первый раз (оформление) договора", 
                L"first_time" 
            }, 
            { 
                L"+20 позиций", 
