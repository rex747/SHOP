// price_tag_printer.h
// =============================================================================
// МОДУЛЬ ПЕЧАТИ ЦЕННИКОВ 100 x 50 мм (горизонтальная ориентация)
// =============================================================================
// ИСПРАВЛЕНИЕ: Функция drawBarcodeVertical теперь правильно рисует
// вертикальные полосы, что соответствует повороту горизонтального
// штрих-кода на 90° по часовой стрелке.
// =============================================================================
#pragma once
#include <windows.h>
#include <string>
...
private:
	...
	static void drawBarcodeVertical(HDC hdc, int x, int yTop, int yBottom, int maxWidthPx,
		const std::string& payload, int barcodeWidthPx, int barcodeHeightPx) {
		if (payload.empty() || yBottom <= yTop || maxWidthPx <= 0 || barcodeWidthPx <= 0 || barcodeHeightPx <= 0) {
			g_logger.warning(L"PriceTagPrinter: drawBarcodeVertical invalid parameters");
			return;
		}

		auto modules = Code128::buildModules(payload);
		if (modules.empty()) {
			g_logger.warning(L"PriceTagPrinter: drawBarcodeVertical: no modules");
			return;
		}

		const int n = static_cast<int>(modules.size());
		g_logger.info(L"PriceTagPrinter: drawBarcodeVertical: n=" + std::to_wstring(n) +
			L", barcodeWidthPx=" + std::to_wstring(barcodeWidthPx) +
			L", barcodeHeightPx=" + std::to_wstring(barcodeHeightPx) +
			L", maxWidthPx=" + std::to_wstring(maxWidthPx) +
			L", areaH=" + std::to_wstring(yBottom - yTop));

		HDC memDC = CreateCompatibleDC(hdc);
		if (!memDC) {
			g_logger.error(L"PriceTagPrinter: drawBarcodeVertical: CreateCompatibleDC failed");
			return;
		}

		HBITMAP hBmp = CreateCompatibleBitmap(hdc, barcodeWidthPx, barcodeHeightPx);
		if (!hBmp) {
			DeleteDC(memDC);
			g_logger.error(L"PriceTagPrinter: drawBarcodeVertical: CreateCompatibleBitmap failed");
			return;
		}

		HGDIOBJ hOldBmp = SelectObject(memDC, hBmp);

		RECT rc = { 0, 0, barcodeWidthPx, barcodeHeightPx };
		HBRUSH hWhite = CreateSolidBrush(RGB(255, 255, 255));
		FillRect(memDC, &rc, hWhite);
		DeleteObject(hWhite);

		ReceiptPrinter::drawBarcodeLine(memDC, 0, barcodeWidthPx, 0, barcodeHeightPx, payload);

		Gdiplus::Bitmap bmp(hBmp, nullptr);
		if (bmp.GetLastStatus() != Gdiplus::Ok) {
			g_logger.error(L"PriceTagPrinter: drawBarcodeVertical: failed to create Gdiplus::Bitmap");
		...
		Gdiplus::Bitmap* pRotated = bmp.Clone(0, 0, barcodeWidthPx, barcodeHeightPx, PixelFormat32bppARGB);
		if (!pRotated) {
			g_logger.error(L"PriceTagPrinter: drawBarcodeVertical: Clone failed");
			SelectObject(memDC, hOldBmp);
			DeleteObject(hBmp);
			DeleteDC(memDC);
			return;
		}
		pRotated->RotateFlip(Gdiplus::Rotate90FlipNone);

		// ---- Рисуем повёрнутый штрих-код с растяжением на всю область ----
		Gdiplus::Graphics graphics(hdc);
		graphics.SetInterpolationMode(Gdiplus::InterpolationModeHighQualityBicubic);

		Gdiplus::RectF destRect(static_cast<Gdiplus::REAL>(x),
			static_cast<Gdiplus::REAL>(yTop),
			static_cast<Gdiplus::REAL>(maxWidthPx),
			static_cast<Gdiplus::REAL>(yBottom - yTop));

		graphics.DrawImage(pRotated, destRect,
			static_cast<Gdiplus::REAL>(0.0f),   // srcX
			static_cast<Gdiplus::REAL>(0.0f),   // srcY
			static_cast<Gdiplus::REAL>(pRotated->GetWidth()),   // srcWidth
			static_cast<Gdiplus::REAL>(pRotated->GetHeight()),  // srcHeight
			Gdiplus::UnitPixel);

		g_logger.info(L"PriceTagPrinter: drawBarcodeVertical: rotated barcode drawn at x=" +
			std::to_wstring(x) + L", yTop=" + std::to_wstring(yTop) +
			L", size=" + std::to_wstring(maxWidthPx) + L"x" + std::to_wstring(yBottom - yTop) + L" px");
